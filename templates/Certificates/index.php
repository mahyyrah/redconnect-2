...
.table { font-size:15px; margin-bottom:0; min-width:950px; }
...
.cert-badge { background:#fff0f0; color:#c1121f; padding:7px 13px; border-radius:20px; font-size:14px; font-weight:700; }
.history-badge { background:#e8f2ff; color:#0d6efd; padding:7px 13px; border-radius:20px; font-size:14px; font-weight:700; }
...
body.dark-mode .cert-badge, body.dark-mode .history-badge { background:#2d2d2d; color:#ff6b6b; }
...
<div class="dashboard-wrapper">
    <aside class="sidebar">
        ...
        <?= $this->Html->link('🏅 Certificates', ['controller'=>'Certificates','action'=>'index'], ['class'=>'active']) ?>
    ...
    <main class="main-content">
        <div class="topbar d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <div>
                <h4 class="mb-1">Certificate Management</h4>
                <p class="text-muted mb-0">Manage certificates generated for completed blood donations.</p>
            </div>
        ...
        <div class="page-card">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
                <div>
                    ...
                    <p class="text-muted page-subtitle mb-0">Certificates issued based on donor donation histories.</p>
                </div>

                <?= $this->Html->link('+ Generate Certificate', ['action'=>'add'], ['class'=>'btn-red']) ?>
            </div>
                    ...
                    <tbody>
                        ...
                        <?php foreach ($certificates as $certificate): ?>
                            <tr>
                                <td><?= $no++ ?></td>
                                <td>
                                    <?php if ($certificate->hasValue('donation_history')): ?>
                                ...
                                <td><span class="cert-badge"><?= h($certificate->certificate_code) ?></span></td>
                                <td><?= h($certificate->issued_at) ?></td>
                                <td class="text-center">
                                    <?= $this->Html->link('View', ['action'=>'view',$certificate->id], ['class'=>'action-link view-btn']) ?>
                                    <?= $this->Html->link('Edit', ['action'=>'edit',$certificate->id], ['class'=>'action-link edit-btn']) ?>
                                    <?= $this->Form->postLink('Delete', ['action'=>'delete',$certificate->id], [
                                        'method'=>'delete',
                        ...
                        <?php if ($certificates->count() == 0): ?>
                            <tr>
                                <td colspan="5" class="text-center text-muted py-4">No certificate records found.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
